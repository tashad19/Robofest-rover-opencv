...
cv2.setTrackbarPos('numDisparities', 'disp', 17)
cv2.setTrackbarPos('uniquenessRatio', 'disp', 0)
...
while True:
    ...
    # load test images
    picL = cv2.imread("imgL.png")
    picR = cv2.imread("imgR.png")
    ...
    # test images converted to grayscale
    picL_gray = cv2.cvtColor(picL, cv2.COLOR_BGR2GRAY)
    picR_gray = cv2.cvtColor(picR, cv2.COLOR_BGR2GRAY)
    ...
    disparity = stereo.compute(picL_gray, picR_gray)
